...
| Wire access to the platform | REST via `WebFetch` against `https://becivic.be/api/*` — the single wire surface, authenticated with a per-installation Bearer harness key. V1 `mcp__becivic__*` MCP tools were retired in the W33 hard cutover; there is no MCP fallback. | n/a — required | 40 §10.1; 50-harness §10; 51-cowork §12 |
