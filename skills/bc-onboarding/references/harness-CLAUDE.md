...
2. **First working session → acknowledge it's their first time, then fetch the canonical and serve the about-you form (once, after the canary).** When the profile is still at defaults, this is a first-time Be Civic user: your canary already acknowledged that it's their first time (§3.0b). Now **introduce and serve the about-you form** — frame it plainly as the first step ("To set you up properly, I'll start with a few questions about your situation"), don't drop it on the user unannounced. The form itself is **required** — it is how the harness learns the user's situation, and a first-time user always gets it. Then:
   ...
   a. **Fetch the canonical first.** Before serving the form, fetch the carried-over procedure's canonical body via `GET ${BASE}/api/processes/<id>` (with the Bearer — §6). You need its frontmatter `inputs:` *now* — to know which fields are required when you **validate** the submission (step c), and for the situation assessment + body walk that follow. (The server uses the same `inputs:` to compose the form's Section 2 for you in step b — you don't assemble it yourself.) If the procedure carried as `intake`, route via `be-civic:bc-discovery` in `process` mode first to resolve a real procedure, then fetch. If the library is unreachable, tell the user and retry rather than silently proceeding without the procedure's inputs.

   b. **Introduce, then fetch + render the server-composed form.** In one short beat, tell the user the form is coming and why ("a few quick questions so I can tailor this to your situation"), then **fetch the ready-made form from the library** — `GET ${BASE}/api/onboarding-form?skill_id=<process-id>&locale=<locale>&mode=first-contact` (with the Bearer — §6). The response carries `form_html`: the server composes Section 1 (core routing) **and** Section 2 (the procedure-specific questions, derived from the same `inputs:`) and localizes it for you. Pass that `form_html` **verbatim** to `mcp__visualize__show_widget` as `widget_code`. **Do not hand-inject Section 2, and do not call `mcp__visualize__read_me`** — the form is shipped, branded, self-contained HTML; read_me is for building widgets, not rendering one the server already built. The form returns its fields as a single `Be Civic onboarding — <field>: <value> · …` chat message.
   - **Fallback only if `/api/onboarding-form` is unreachable:** render the static `${SUBSTRATE_ROOT}/skills/bc-onboarding/references/onboarding.<locale>.html` (read via `bash` — it is a plugin-install asset, host-`Read`-invisible) and hand-inject Section 2 from the step-a `inputs:` per the runtime-insertion note in the file. This is the legacy path; prefer the server form.
