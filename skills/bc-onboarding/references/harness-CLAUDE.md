...
**Reaching each surface (Cowork host/VM split).** `${SUBSTRATE_DATA}` (and its `${SUBSTRATE_STATE}` child) is the user-mounted project folder — read and write it with the host `Read`/`Write`/`Edit` tools. `${SUBSTRATE_ROOT}` is the plugin install dir, which lives **only on the bash side** (the VM mount) — read its assets (scripts, schemas, references) with `bash` (`cat`, `python3 …`), not the host `Read` tool, which cannot see it. A host-`Read` of a `${SUBSTRATE_ROOT}/…` path failing "outside connected folders" is this split, not a missing file.
