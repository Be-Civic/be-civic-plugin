...
The bullets above are the conversational form; the **canonical, verbatim privacy answer** lives at `data/privacy-snippet.md` (the gate skill delivers it verbatim for a bare "what does Be Civic do with my data?"). Keep these aligned with it — do not let a paraphrase drift from the canonical claims (what leaves the machine, the 48-hour cancel window, the routing/usage-stats split).
